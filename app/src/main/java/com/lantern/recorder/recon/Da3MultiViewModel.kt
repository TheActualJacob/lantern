 ...
 *   - per-view depth `(N, res, res)` — distance (z), DA3 units, consistent across views,
 *   - per-view confidence `(N, res, res)`,
 *   - DA3's own estimated **extrinsics** `(N, 3, 4)` world->camera `[R|t]` (OpenCV),
 *   - DA3's own estimated **intrinsics** `(N, 3, 3)` at `res x res`.
 *
 * This is what makes a scanned box reconstruct as a coherent box instead of the bent, convex
 * "banana" the mono per-frame + affine path produces (verified host-side; see
 * docs/RECON_QUALITY_DIAGNOSIS.md). Because DA3 supplies the poses + intrinsics, the caller does
 * not need ARCore poses for the *shape* — only (optionally) ARCore depth to fix metric scale.
 *
 * The input rank/shape is frozen at export, so [numViews] and [res] are fixed (read from the
 * `_nN_rRES` filename). The caller must pass exactly [numViews] views (pad by repetition if fewer
 * were captured). Heavy (cross-view attention over N views); call off the main thread, once.
 */
class Da3MultiViewModel private constructor(
    ...
    /** Letterbox placement of one source image inside the [res] square (for mask alignment). */
    class ViewGeom(val padX: Int, val padY: Int, val newW: Int, val newH: Int)

    /**
     * Multi-view result. [depth]/[conf] are flat `N*res*res`; [extrinsics] flat `N*12` (3x4
     * row-major, world->cam); [intrinsics] flat `N*9` (3x3 row-major). [geom] gives each input
     * view's letterbox so masks/colors can be sampled into the same `res x res` grid as [depth].
     */
    class Result(
        val n: Int,
        val res: Int,
    ...
    fun infer(argbs: List<ImageUtils.Argb>): Result? {
        ...
        return try {
            ...
            val tensor = org.pytorch.executorch.Tensor.fromBlob(
                input, longArrayOf(1, numViews.toLong(), 3, res.toLong(), res.toLong()),
            )
    ...
    class Letterbox(srcW: Int, srcH: Int, target: Int) {
    ...
    companion object {
        ...
        /** Parse `_n<N>_r<RES>` from the `.pte` filename (input rank is fixed at export). */
        private fun parseShape(path: String): Pair<Int, Int>? {
            val m = Regex("_n(\\d+)_r(\\d+)\\.pte$").find(path) ?: return null
            return m.groupValues[1].toInt() to m.groupValues[2].toInt()
        }

        /**
         * Load a multi-view DA3 `.pte`. [numViews]/[res] are read from the `_nN_rRES` filename
         * suffix (they must match the export). Returns null if the file is missing, the suffix
         * can't be parsed, or the runtime can't load it (caller falls back to mono).
         */
        fun loadOrNull(modelPath: String): Da3MultiViewModel? {
            val shape = parseShape(modelPath)
            if (shape == null) {
                Log.w(TAG, "Da3MultiView: filename lacks _n<N>_r<RES> suffix ($modelPath); skipping")
                return null
            }
            return try {
                val module = Module.load(modelPath)
                Log.i(TAG, "Loaded multi-view DA3: $modelPath (N=${shape.first}, res=${shape.second})")
                Da3MultiViewModel(module, shape.first, shape.second)
            } catch (t: Throwable) {
